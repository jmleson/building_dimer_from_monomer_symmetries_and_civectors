...
class TestDimerDeterminant(unittest.TestCase):
    ...
    def test_ci_vector(self):
        ...
        try:
            eq = self.d1.latex_ci_equation(ordering=CI_ORDERING.molpro)
            raise Exception("should raise error because zero-orbitals are not set")
        except:
            pass

        try:
            eq = self.d2.latex_ci_equation(ordering=CI_ORDERING.molpro)
            raise Exception("should raise error because zero-orbitals are not set")
        except:
            pass
        ...
        eq = self.d3.latex_ci_equation(ordering=CI_ORDERING.molpro)
        # 1e- = "+ag", "-b1u", "+b2g", "-au"], zero = ["b3g", "b2u"]
        assert eq == r"-\left|"+ "a202aa0a" + r"\right|"

        eq = self.d4.latex_ci_equation(ordering=CI_ORDERING.molpro)
        assert eq == r"+\left|" + "a202aa0a" + r"\right|"
