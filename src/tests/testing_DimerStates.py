...
class TestDimerStates(unittest.TestCase):
    ...
    def test_generation(self):
        ...
        assert "S \bullet Q + Q \bullet S" in strings
        assert "S \bullet Q - Q \bullet S" in strings
        assert "i^3 B_{2u} \bullet i^3 B_{2u}" in strings
        assert "i^3 B_{2u} \bullet i^3 B_{2u} + i^3 B_{2u} \bullet i^3 B_{2u}" in strings
        assert "i^3 B_{2u} \bullet i^3 B_{2u} - i^3 B_{2u} \bullet i^3 B_{2u}" in strings
        assert "i^3 B_{2u} \bullet i^3 B_{3u} + i^3 B_{3u} \bullet i^3 B_{2u}" in strings
        assert "i^3 B_{2u} \bullet i^3 B_{3u} - i^3 B_{3u} \bullet i^3 B_{2u}" in strings
        assert "i^3 B_{2u} \bullet i^3 B_{3u} + i^3 B_{3u} \bullet i^3 B_{2u}" in strings
        assert "i^3 B_{2u} \bullet i^3 B_{3u} - i^3 B_{3u} \bullet i^3 B_{2u}" in strings
        assert "i^3 B_{2u} \bullet i^3 B_{2u}" in strings
        ...
        assert "i^3 B_{2u} \bullet i^3 B_{3u} + i^3 B_{3u} \bullet i^3 B_{2u}" in strings
        assert "i^3 B_{2u} \bullet i^3 B_{3u} - i^3 B_{3u} \bullet i^3 B_{2u}" in strings
        assert "i^3 B_{3u} \bullet i^3 B_{3u}" in strings
        assert "i^3 B_{3u} \bullet i^3 B_{3u} + i^3 B_{3u} \bullet i^3 B_{3u}" in strings
        assert "i^3 B_{3u} \bullet i^3 B_{3u} - i^3 B_{3u} \bullet i^3 B_{3u}" in strings
        assert "i^3 B_{3u} \bullet i^3 B_{3u}" in strings

        assert "i^3 B_{2u} \bullet i^3 B_{2u}" == strings[2]
        ...
        # assert "i^3 B_{2u} * i^3 B_{2u} + i^3 B_{2u} * i^3 B_{2u}"  == strings[3]
    ...
    def test_end_results(self):
        ...
        sq_plus, simplified_ci_vectors = self.find_dimer_state_by_label_and_set_up_for_testing(dimer_states=dimer_states, label="S \bullet Q + Q \bullet S")
        ...
        sq_minus, simplified_ci_vectors = self.find_dimer_state_by_label_and_set_up_for_testing(dimer_states=dimer_states, label="S \bullet Q - Q \bullet S")
        ...
        root1, simplified_ci_vectors = self.find_dimer_state_by_label_and_set_up_for_testing(dimer_states, label ="i^3 B_{2u} \bullet i^3 B_{2u}")
        ...
        root2, simplified_ci_vectors = self.find_dimer_state_by_label_and_set_up_for_testing(dimer_states,
                                                       label="i^3 B_{2u} \bullet i^3 B_{2u} + i^3 B_{2u} \bullet i^3 B_{2u}")
