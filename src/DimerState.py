...
class DimerState:
    ...
    def __init__(self, monomer_state_1: MonomerState, monomer_state_2: MonomerState, combination:SIGN, point_group:POINTGROUP):
    ...
    def to_latex(self, ordering:CI_ORDERING, detailed:bool=False):
        ...
        eq += self.written_in_monomer_ci_vectors(ordering=ordering)
        if detailed:
    ...
    def written_in_monomer_ci_vectors(self, ordering:CI_ORDERING):
        eq = get_array_environment([d.written_in_monomer_ci_vectors(ordering=ordering) for d in self.dimer_occupations],
                                   breaking_after=4)
    ...
    def monomer_determinants(self, multiplied_out:bool):
        eq = get_array_environment([i.monomer_determinants(multiplied_out=multiplied_out) for i in self.dimer_occupations],
                                   breaking_after=6 if not multiplied_out else 2)
        return get_expression_as_latex_formula(eq, latex_equation_types.MULTLINE)
