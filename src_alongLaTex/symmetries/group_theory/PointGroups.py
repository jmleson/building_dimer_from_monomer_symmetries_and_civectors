...
class POINTGROUP(Enum):
    ...
    def __new__(cls, value):
        ...
        if value == "d2h":
            ...
            obj.irreduzible_representations_molpro_ordered_monomer = ('ag', 'b3u', 'b2u', 'b1g', 'b1u', 'b2g', 'b3g', 'au')
            ...
            obj.irreduzible_representations_molpro_ordered_monomer = ('a1', 'a1*', 'b1', 'b1*', 'b2', 'b2*', 'a2', 'a2*')
            obj.mo_pairs = {  # !sortiert; ! brauchen Vorzeichen
                "a2*^{l}":  "+b1*+a2*",#*
                "b2*^{l}":  "+a1*+b2*",#*
                "a2^{l}":   "+b1+a2",
            ...
            obj.total_symmetric = "ag"
            obj.irreduzible_representations_molpro_ordered = ('ag', 'ag*', 'bg', 'bg*', 'bu', 'bu*', 'au', 'au*')#Dimer
            obj.irreduzible_representations_molpro_ordered_monomer = ('a1', 'a1*', 'b1', 'b1*', 'b2', 'b2*', 'a2', 'a2*')#Monomer
            obj.mo_pairs = {  # !sortiert; ! brauchen Vorzeichen
                "a2*^{l}":  "+au*+bg*",#*
                "b2*^{l}":  "+ag*+bu*",#*
                "a2^{l}":   "+au+bg",
    ...
    def product(self,factor_1:str, factor_2:str):
        ...
        if self.value == "d2h":
            if (factor_1 not in self.irreduzible_representations_molpro_ordered or
                    factor_2 not in self.irreduzible_representations_molpro_ordered):
        ...
        if self.value == "c2h":
            ...
            if (factor_1 not in self.irreduzible_representations_molpro_ordered or
                    factor_2 not in self.irreduzible_representations_molpro_ordered):
                if factor_1 not in self.irreduzible_representations_molpro_ordered_monomer or factor_2 not in self.irreduzible_representations_molpro_ordered_monomer:
