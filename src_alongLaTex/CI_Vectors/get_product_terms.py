...
from src_alongLaTex.CI_Vectors.count import combine_sequences
from src_alongLaTex.CI_Vectors.write_in_ci_vectors import write_in_ci_vectors
...
from src_alongLaTex.symmetries.linear_combinations.linear_combination_monomer_states import get_monomer_combinations, \
...
def draw(l: linear_combination_of_dimeroccstates, point_group: POINTGROUP, detailed:bool=True):
    ...
    content += l.draw() + "\n\n"
    content_tmp, dimer_ci_vectors = write_in_ci_vectors(l=l, break_after_number_of_terms=4)
    if detailed:
        content += content_tmp
    # print(content_tmp)
